      ...
      <div className="max-w-[1490px] mx-auto px-10 py-[120px]">
        <div className="flex flex-col items-center xl:items-start xl:flex-row xl:justify-between gap-12">
          <div className="w-full xl:w-auto flex flex-col items-center xl:items-start">
            ...
            <div className="relative w-full xs:w-[380px] mt-11">
              <input 
                type="email"
                placeholder="Email"
                className="rounded-full w-full pl-6 pr-[140px] py-4 bg-primary outline-none text-white text-base xs:text-g placeholder-white"
              />
              ...
              {footerLinksColumnOne.map((item, i) => (
                <div key={i} className="text-white max-w-screen-md md:max-w-[340px] flex-1 md:flex-auto">
                  ...
                  <ul className="flex flex-col gap-4 ">
                    {item.links.map((it, i) => (
                      <div key={i} className="w-full flex mx-auto justify-center xl:justify-start">
                        <li
                        key={`list-item-${i}`}
                  ...
                  <ul className="flex flex-col gap-4 ">
                    {item.links.map((it, i) => (
                      <div key={i} className="w-full flex mx-auto justify-center xl:justify-start">
                        <li
                        key={`list-item-${i}`}
                        className={`font-medium text-sm md:text-base ${i !== 1 && 'cursor-pointer'} text-center xl:text-start w-fit`}
                        >
                          {it}
                        </li>
                      </div>
